...
module lexer =
  ...
  let rec private lexer nextc input tokens = 
      ...
      |  _  when List.contains nextc ['0'..'9'] ->
               ...
               if (lookahead = '.') then
                let (afterpoint,lookahead) = collectIntLiteral (nextChar input) input ""
                let realliteral = (literal + "." + afterpoint) 
                lexer lookahead input (("real_literal:" + realliteral) :: tokens)
               else
